...
object SecretAura : Module(
    ...
    private val swapOn = BooleanSetting("Item swap", false, description = "Makes secret aura swap to slot on click.")
    private val swapBack = BooleanSetting("Swap back", false, description = "Makes secret aura swap back to previous item after swapping.").withDependency { swapOn.enabled }
    private val swapSlot: NumberSetting = NumberSetting("Swap item slot", 1.0, 1.0, 9.0, 1.0, description = "Slot for secret aura to swap to.").withDependency { swapOn.enabled }
    private val swingOn = BooleanSetting("Swing hand", false, description = "Makes secret aura swing hand on click.")
            ...
            if (blockState.block === Blocks.chest || blockState.block === Blocks.trapped_chest) {
                ...
                if (eyePos.distanceTo(Vec3(block)) <= auraRange.value) {
                    val movingObjectPosition: MovingObjectPosition = collisionRayTrace(
                        ...
                        ?: continue
                    if (swapOn.value && mc.thePlayer.inventory.currentItem != swapSlot.value.toInt() - 1) {
                    ...
                    if (!mc.thePlayer.isSneaking && swingOn.value) mc.thePlayer.swingItem()
                    blocksCooldown[block] = Date().time
                    return
                }
                ...
                else if (orientation == EnumOrientation.SOUTH) AxisAlignedBB(0.3125, 0.2, 0.0, 0.6875, 0.8, 0.375)
                else if (orientation == EnumOrientation.NORTH) AxisAlignedBB(0.3125, 0.2, 0.625, 0.6875, 0.8, 1.0)
                else if (orientation == EnumOrientation.UP_Z || orientation == EnumOrientation.UP_X) AxisAlignedBB(0.25, 0.0, 0.25, 0.75, 0.6, 0.75)
                else if (orientation == EnumOrientation.DOWN_X || orientation == EnumOrientation.DOWN_Z) AxisAlignedBB(0.25, 0.4, 0.25, 0.75, 1.0, 0.75)
                else continue
                ...
                if (eyePos.distanceTo(Vec3(block)) <= auraRange.value) {
                    val movingObjectPosition: MovingObjectPosition =
                        collisionRayTrace(block, aabb, eyePos, centerPos)
                            ?: continue
                    if (swapOn.value && mc.thePlayer.inventory.currentItem != swapSlot.value.toInt() - 1) {
                        prevSlot = mc.thePlayer.inventory.currentItem
                        mc.thePlayer.inventory.currentItem = swapSlot.value.toInt() - 1
                ...
                val tileEntity: TileEntity = mc.theWorld.getTileEntity(block) as? TileEntitySkull ?: continue
                ...
                if (!Objects.equals(profileId, "e0f3e929-869e-3dca-9504-54c666ee6f23")) {
                    ...
                    else if (mc.theWorld.getBlockState(block.down()).block === Blocks.redstone_block || mc.theWorld.getBlockState(
                            block.north()
                        ).block === Blocks.redstone_block || mc.theWorld.getBlockState(block.south()).block === Blocks.redstone_block || mc.theWorld.getBlockState(
                            block.west()
                        ).block === Blocks.redstone_block || mc.theWorld.getBlockState(block.east()).block === Blocks.redstone_block
                    ) {
                        redstoneKey = false
                ...
                val facing = blockState.properties[BlockSkull.FACING] as EnumFacing
                val aabb = if (facing == EnumFacing.NORTH) AxisAlignedBB(0.25, 0.25, 0.5, 0.75, 0.75, 1.0)
                else if (facing == EnumFacing.SOUTH) AxisAlignedBB(0.25, 0.25, 0.0, 0.75, 0.75, 0.5)
                else if (facing == EnumFacing.WEST) AxisAlignedBB(0.5, 0.25, 0.25, 1.0, 0.75, 0.75)
                else if (facing == EnumFacing.EAST) AxisAlignedBB(0.0, 0.25, 0.25, 0.5, 0.75, 0.75)
                ...
                if (mc.theWorld.getBlockState(block.up()).block === Blocks.skull || mc.theWorld.getBlockState(block.north()).block === Blocks.skull || mc.theWorld.getBlockState(
                        block.south()
                    ).block === Blocks.skull || mc.theWorld.getBlockState(block.west()).block === Blocks.skull || mc.theWorld.getBlockState(
                        block.east()
                    ).block === Blocks.skull
                ) {
                    redstoneKey = false
    ...
    @SubscribeEvent
    fun onPacketReceive2(event: ReceivePacketEvent) {
